...
// ── Users ──────────────────────────────────────────────────────────────────
// Note: field names follow better-auth conventions (name, image, emailVerified)
// Our API layer maps name→display_name and image→avatar_url for frontend compat.
// Before running drizzle-kit push for the first time, run:
//   ALTER TABLE sessions RENAME TO focus_sessions;
export const users = pgTable("users", {
  id:                     uuid("id").primaryKey().defaultRandom(),
  name:                   text("name").notNull().default(""),            // better-auth: display name
  email:                  text("email").notNull().unique(),
  emailVerified:          boolean("email_verified").notNull().default(false),
  image:                  text("image"),                                  // better-auth: avatar URL
  // Our custom fields
...
// Note: 'sessions' is used by better-auth for auth sessions.
// The old Go 'sessions' table (focus sessions) is renamed to 'focus_sessions'.
...
export const groups = pgTable("groups", {
  ...
  type:         text("type").notNull().default("group"), // 'group' | 'duo'
  createdAt:    timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
})
...
// ── Focus Sessions (renamed from 'sessions' in Go DB) ─────────────────────
...
export const recurringCommitments = pgTable("recurring_commitments", {
  ...
  color:        text("color").notNull().default("green"), // green | indigo | amber
  description:  text("description"),
  targetMinDay: integer("target_min_day"),
  periodDays:   integer("period_days").notNull().default(30),
  startDate:    date("start_date").notNull().default(sql`CURRENT_DATE`),
  endDate:      date("end_date").notNull(),
  status:       text("status").notNull().default("active"), // active | archived | pending_review
  createdAt:    timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
...
export const commitmentShares = pgTable("commitment_shares", {
  ...
  weekStart:    date("week_start"),   // null = full history; set for week-specific share links
  createdAt:    timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
...
export const dailyScores = pgTable("daily_scores", {
  ...
  userId:    uuid("user_id").notNull().references(() => users.id, { onDelete: "cascade" }),
  date:      date("date").notNull(),
  score:     smallint("score").notNull().default(0), // 0-5
...
// ── Declarations & Commitments (daily declaration flow) ───────────────────
...
export const commitments = pgTable("commitments", {
  ...
  intensity:     text("intensity").notNull().default("firm"),  // soft | firm | non_negotiable
  tag:           text("tag"),  // work | learning | health | relationships | other
  groupId:       uuid("group_id"),
  status:        text("status").notNull().default("pending"),  // pending | completed | carried | dropped
  slipCount:     smallint("slip_count").notNull().default(0),
  honestScore:   smallint("honest_score"),  // 1-3
